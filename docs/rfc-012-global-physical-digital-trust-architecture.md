...
**Status:** Accepted (Phases 0–9 complete on `dev`; Phase 10 pending)  
**Created:** 2026-09-22  
**Updated:** 2026-09-26  
...
**Entry for a new contributor or agent:** read [`VISION.md`](VISION.md) first (stages 0–10, where we are), then §8.1 (Handoff) of this document, then §8 (phase table), §9 (migration map), §10 (PDA sketch). Do not invent source counts — see `terra-core/docs/architecture.md` (verified 2026-09-26).
...
| **9** | Physical infrastructure | **Complete** (2026-09-26) | `device_identity.rs`: `DeviceIdentity` PDA (`["device_identity", owner, device_nonce(u16)]`) binding device key + capture source + capability bitfield (GNSS/PHOTO/VIDEO/LIDAR/IMU/RTK/SCANNER_3D/THERMAL) + calibration hash + status (ACTIVE/SUSPENDED/REVOKED — terminal); 6 instructions (register / update / rotate key / set status / set calibration / validator verify); 6 events; 5 errors (6220–6224, reuses 6182/6014); 6 unit + 2 BPF tests (`phase9_*`); IDL 154/63/141/225 |
| **10** | Cross-border + privacy | Pending | Jurisdiction bindings, ZK identity/ownership, selective disclosure |

Phase 9 is complete on `dev` (2026-09-26), after Phase 8 (2026-09-24) and the RFC-012 legacy sweep / B6 program-boundary split / P0-2 removal-endorsement path (2026-09-25). Before starting Phase 10 on a clean checkout, confirm the checked-in IDL matches source (`make idl` — current as of 2026-09-26 it is **154/63/141/225** with Phase 2 validator-profile PDAs + Phase 3 task PDAs + Phase 4 ObservationV2 + Phase 5 EvidenceManifest/EvidenceArtifact + Phase 6 route_task + Phase 7 fraud/review/restriction/appeal PDAs + Phase 8 fee/coverage/quote/escrow/allocation PDAs + Phase 9 device_identity PDAs, post-legacy-sweep + B6 + P0-2) and clear remaining SECURITY.md mainnet items (RFC-005 reconfirm, ZK audit) — see §8.1.

---

## 8.1. Handoff: Phase 9 complete; how to start Phase 10
...
**Phase 9 delivered (2026-09-26):** `device_identity.rs` implements physical infrastructure — an on-chain registry of capture devices. Account `DeviceIdentity` (PDA seeds `["device_identity", owner, device_nonce(u16 LE)]`, one nonce per device per owner) records `device_key` (device's own signing key, distinct from the owner wallet — Design Rule 7), `source` (the Phase 4 capture class it produces, validated by `observation_v2::is_valid_observation_source`), `capabilities` (bitfield GNSS/PHOTO/VIDEO/LIDAR/IMU/RTK/SCANNER_3D/THERMAL), `metadata_ref` (≤128 bytes, model/firmware URI or IPFS CID), `calibration_hash`/`calibrated_at`, `status` (ACTIVE 0 / SUSPENDED 1 / REVOKED 2 — REVOKED terminal), and `verified`/`verified_by`/`verify_version`. 6 instructions: `register_device(device_nonce, device_key, source, capabilities, metadata_ref)` (permissionless, owner pays rent, starts UNVERIFIED), `update_device(capabilities, metadata_ref)` / `rotate_device_key(new_device_key)` / `set_device_status(status)` / `set_device_calibration(calibration_hash, calibrated_at)` (owner-only via `device.owner == owner` constraint + revoked guard), `verify_device` (requires the signer's `ValidatorProfile` PDA with `profile.wallet == validator` — flips `verified`, bumps `verify_version`; claim → fact, same pattern as RFC-006 bindings). Events: `DeviceRegistered`, `DeviceUpdated`, `DeviceKeyRotated`, `DeviceStatusChanged`, `DeviceCalibrationSet`, `DeviceVerified`. Errors appended after `NothingToRefund`: `DeviceMetadataTooLong` (6220), `InvalidDeviceKey` (6221), `InvalidDeviceStatus` (6222), `DeviceRevoked` (6223), `NotDeviceOwner` (6224); reuses `InvalidObservationSource` (6182) and `EmptyContentHash` (6014); 6 lib unit tests + 2 BPF integration tests (`phase9_*`); IDL 154/63/141/225. Off-chain over these accounts (not in this phase): hardware attestation (Android Key Attestation / secure-enclave certs bound to `device_key`) and observation-envelope signature verification.

**Immediate next actions for Phase 10:**
1. Read §9 (Migration Map) and §10 (PDA sketch) — Phase 9 delivered the device registry; Phase 10 is cross-border + privacy (jurisdiction bindings, ZK identity/ownership proofs, selective disclosure) and must reuse Phase 6's jurisdiction soft-pass stub plus RFC-006/RFC-011 machinery.
2. Checked-in IDL was refreshed in A2 (119/44/108/160), Phase 2 (128/49/115/169), Phase 3 (134/52/120/182), Phase 4 (135/53/121/184), Phase 5 (137/55/123/187), Phase 6 (138/55/124/191), Phase 7 (147/59/133/206), Phase 8 (153/64/139/220), legacy sweep (146/62/134/220), B6 (147/62/135/220), P0-2 (148/62/135/220, 2026-09-25), and Phase 9 (**154/63/141/225**, 2026-09-26 — current); re-run `make idl` after further program edits. Close remaining SECURITY.md items (RFC-005 reconfirm, ZK audit) if touching staking/ZK.
3. When adding accounts/instructions/events/errors: append (never remove/reorder) `TerraError` variants, update `rfc012_structure.rs` expectations if §8/§10 counts changed, root/`terra-core` README source counts, and run `make idl`.
4. Ship each phase with unit tests + BPF integration tests per §11.

**Phase order (next):** 10 (cross-border/privacy) — the final RFC-012 phase. Do not skip the migration map dual-write rules in §9.
...
DeviceIdentity              seeds: ["device_identity", owner, device_nonce]  (u16 LE nonce)
