...
pub fn migrate_rights<'a>(ctx: Context<'a, super::MigrateRights<'a>>) -> Result<()> {
    ...
    for pair in ctx.remaining_accounts.chunks(2) {
        ...
        // Deserialize the old Rights account via Anchor's AccountDeserialize
        // instead of manual offset slicing. This is safe against layout changes
        // and ensures discriminator + borsh body are validated as a unit.
        ...
        let rights_kind = old_rights.rights_kind;
        let holder = old_rights.holder;
        let granter = old_rights.granter;
        let created_at = old_rights.created_at;
        let expires_at = old_rights.expires_at;
        let notes = old_rights.notes;
        let status = old_rights.status;
        let grace_period_secs = old_rights.grace_period_secs;
